...
class PurchaseController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Mendapatkan nomor terakhir untuk tanggal saat ini
            $lastSequence = PurchaseOrder::whereDate('created_at', $currentDate)->count() + 1;

            // Format nomor delivery order
            ...
            // Simpan gambar dengan nama acak di lokasi yang diinginkan
    ...
    public function update(Request $request, string $poId)
    {
        ...
        try {
            ...
            if ($request->hasFile('attachment_image')) {
                ...
                $request->file('attachment_image')->storeAs('public/images/InventoryTransferIn', $originalImageName);

                // Delete old attachment if exists
                if ($purchaseOrder->attachment_image) {
                    Storage::delete('public/images/PurchaseOrder/' . $purchaseOrder->attachment_image);
                    Storage::delete('public/images/InventoryTransferIn/' . $purchaseOrder->attachment_image);
                }

                $data['attachment_image'] = $originalImageName;
            } else {
                // If attachment not changed, use the existing one
                $data['attachment_image'] = $purchaseOrder->attachment_image;
            }

            $this->purchaseOrderRepository->update($poId, $data);

            $updatedpurchaseOrder = $this->purchaseOrderRepository->find($poId);

            return response()->json(['message' => 'PurchaseOrder updated successfully', 'data' => $updatedpurchaseOrder], 200);
        } catch (\Exception $e) {
