...
class InventoryController extends Controller
{
    ...
    //         $lastSequence = Inventory::whereDate('created_at', $currentDate)->count() + 1;
    ...
    //         $inventory = $this->inventoryRepository->create($purchaseOrderData);

    //         return response()->json(['Message' => 'success create new Inventory', 'data' => $inventory], 201);
    //     } catch (\Exception $e) {
    ...
    //         }
    //         return response()->json(['error' => 'Failed to create Inventory. ' . $e->getMessage()], 422);
    //     }
    // }
    ...
    public function index(Request $request)
    {
        try {
            ...
            return response()->json(['Message' => 'success fetch Inventories', 'data' => $inventories], 200);
        } catch (\Exception $e) {
            return response()->json(['error' => 'Failed to fetch Inventories. ' . $e->getMessage()], 500);
        }
    }
    ...
    public function show(string $inventoryId)
    {
        try {
            ...
            return response()->json(['message' => 'Inventory retrieved successfully', 'data' => $inventory], 200);
        } catch (\Exception $e) {
            return response()->json(['error' => 'Failed to retrieve Inventory. ' . $e->getMessage()], 422);
        }
    }
    ...
    //         $purchaseOrder = $this->inventoryRepository->find($poId);
    ...
    //         $po =  $this->inventoryRepository->update($poId, $data);
    ...
    {
        try {
            $purchaseOrder = $this->inventoryRepository->delete($inventoryId);
